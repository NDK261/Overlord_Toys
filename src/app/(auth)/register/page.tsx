...
function getStrength(pw: string) {
  ...
  if (pw.length < 14) return { level: 3, label: "STRONG",   color: "#6FF7E8" };
  return               { level: 4, label: "FORTRESS", color: "#4edbcd" };
}
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; }
        ...
        .seg { transition: background 0.35s, box-shadow 0.35s; }
        ...
        <div className="auth-left">
          <div className="auth-panel">
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8, textAlign: "center" }}>
              ...
              }}>Join the Collective</h1>
              <p style={{
                fontFamily: "Inter, sans-serif", fontSize: 14,
                color: "rgba(212,229,236,0.45)",
              }}>Create your collector&apos;s account</p>
            </div>
            ...
            <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              <div style={{ position: "relative" }}>
                <span className="ms" style={{
                  ...
                  transform: "translateY(-50%)", color: "rgba(111,247,232,0.45)",
                }}>person</span>
                <input
              ...
              <div style={{ position: "relative" }}>
                <span className="ms" style={{
                  position: "absolute", left: 13, top: "50%",
                  transform: "translateY(-50%)", color: error?.toLowerCase().includes("email") ? "rgba(239, 68, 68, 0.45)" : "rgba(111,247,232,0.45)",
                }}>mail</span>
                <input
                  type="email" value={email} onChange={(e) => setEmail(e.target.value)}
                  placeholder="Email Address" required autoComplete="email"
                  style={{...inputStyle, borderColor: error?.toLowerCase().includes("email") ? "rgba(239, 68, 68, 0.3)" : inputStyle.borderColor}} 
                ...
                {password.length > 0 && (
                  <div>
                    <div style={{ display: "flex", gap: 6 }}>
                      {[1, 2, 3, 4].map((l) => (
                        <div key={l} className="seg" style={{
                          flex: 1, height: 3, borderRadius: 4,
                          background: l <= strength.level ? strength.color : "rgba(255,255,255,0.1)",
                        }} />
                      ))}
                    </div>
                    <p style={{
                      fontFamily: "Inter, sans-serif", fontSize: 11,
                      letterSpacing: "0.15em", color: strength.color, marginTop: 4,
                    }}>
              ...
              <div style={{ position: "relative" }}>
                <span className="ms" style={{
                  ...
                  color: confirm.length > 0 ? (matched ? "#6FF7E8" : "#ef4444") : "rgba(111,247,232,0.45)",
                }}>
                  {confirm.length > 0 ? (matched ? "check_circle" : "error") : "lock"}
                </span>
                <input
                  ...
                  style={{
                    ...
                    borderColor: confirm.length > 0
                      ? (matched ? "rgba(111,247,232,0.5)" : "rgba(239,68,68,0.4)")
                      : "rgba(111,247,232,0.15)",
                  }}
                  onFocus={onFocus} onBlur={onBlur}
                />
                <button type="button" className="vis-btn" onClick={() => setShowCf(!showCf)}
                  style={{ position: "absolute", right: 14, top: "50%", transform: "translateY(-50%)", zIndex: 1 }}>
                  <span className="ms">{showCf ? "visibility_off" : "visibility"}</span>
                </button>
              </div>
              ...
              <label style={{
                ...
                <input
                  ...
                  style={{ marginTop: 3, accentColor: "#6FF7E8", width: 14, height: 14, flexShrink: 0 }}
                />
                <span style={{
                  fontFamily: "Inter, sans-serif", fontSize: 13,
                  color: "rgba(212,229,236,0.6)", lineHeight: 1.5,
                }}>
                  I agree to the{" "}
                  <Link href="/terms" style={{ color: "#6FF7E8", textDecoration: "none" }}>Terms of Service</Link>
                  {" "}and{" "}
                  <Link href="/privacy" style={{ color: "#6FF7E8", textDecoration: "none" }}>Privacy Policy</Link>
                </span>
              </label>

              <button type="submit" className="cta-btn" disabled={loading || !agreed}>
            ...
            <p style={{ textAlign: "center", fontFamily: "Inter, sans-serif", fontSize: 13, color: "rgba(212,229,236,0.5)" }}>
              Already a member?{" "}
              <Link href="/login" style={{ color: "#6FF7E8", textDecoration: "none", fontWeight: 600 }}>
                Sign In →
              </Link>
            </p>
