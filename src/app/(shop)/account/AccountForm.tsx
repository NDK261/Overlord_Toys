...
const ACCOUNT_FORM_COPY = {
  en: {
    success: "Profile updated successfully.",
    error: "Profile update failed.",
    fullName: "Full Name",
    fullNamePlaceholder: "Name of Collector",
    email: "Email",
    immutable: "// Immutable",
    phone: "Phone Number",
    phonePlaceholder: "+84...",
    address: "Shipping Address",
    addressPlaceholder: "Shipping deployment sector...",
    saving: "Syncing...",
    save: "Save Changes",
  },
  vi: {
    ...
    fullName: "Họ và tên",
    fullNamePlaceholder: "Tên người sưu tầm",
    email: "Email",
    immutable: "// Không thể đổi",
    phone: "Số điện thoại",
    phonePlaceholder: "+84...",
...
export function AccountForm({
  ...
  const copy = ACCOUNT_FORM_COPY[settings.shopping.language];
  ...
  const handleSaveProfile = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setStatus({ type: "error", message: err.message || copy.error });
    } finally {
      setIsSaving(false);
    }
  };
    ...
    <form className="space-y-8" onSubmit={handleSaveProfile}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div className="space-y-2">
          <label className="text-[10px] font-black uppercase tracking-[0.2em] text-on-surface-variant/70 px-1 ml-1">
            {copy.email}
            <span className="text-[#6FF7E8]/50 ml-2">{copy.immutable}</span>
          </label>
          <input
