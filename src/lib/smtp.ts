...
const DEFAULT_FROM_EMAIL = "Overlord Toys <no-reply@overlordtoys.com>";
...
function getTransporter() {
  ...
  if (!host || !port || !user || !pass) {
    throw new Error("Missing SMTP credentials in .env.local");
  }
...
export async function sendOrderConfirmationEmail(
  ...
  const from = process.env.SMTP_FROM_EMAIL || DEFAULT_FROM_EMAIL;
